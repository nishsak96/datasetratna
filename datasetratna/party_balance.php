...
<!--   <form class="form-horizontal" name="regForm" method="post">
   ...
   $c=array();
     ...
     $querymain1="SELECT distinct Sheet2.name FROM Sheet2 UNION SELECT distinct Sheet1.name FROM Sheet1 order  by name";
		$querymain2="SELECT distinct name FROM `Sheet1`  ORDER BY `date` desc";
		$result=mysqli_query($dbconnnect,$querymain1);
		//$issue=mysql_fetch_assoc($result);
		$issuemain = array();

		while($row = mysqli_fetch_assoc($result)){
		    array_push($issuemain, $row);
		  //  print_r($issuemain);
		    //echo "<br>";
		}
		///echo $r=mysql_num_rows($result);
		/*$result=mysqli_query($dbconnnect,$querymain2);
		//$recept=mysql_fetch_array($result);
		//print_r($issue);
		$receptmain = array();

		while($row = mysqli_fetch_assoc($result)){
		    array_push($receptmain, $row);
		    //print_r($recept);
		    //echo "<br>";
		}*/
   ...
   ///print_r($a[0]);
   $query="SELECT SUM(pure_amount) FROM `Sheet1` WHERE name='$tempname'";
   $result=mysqli_query($dbconnnect,$query);
