	    		...
	    		if($z==1)
	    		{
		    		echo '<div>
		    			<ul class="nav navbar-nav">
		    				...
		    				<li><a href="deleted_entries.php">DeletedEntries</a></li>
  ...
  <select class="form-control" id="sel1" name="option">
    ...
    <option>Receipt</option>
     ...
     <div class="form-group">
      ...
      <div class="col-sm-5">
        <input type="text" class="form-control" name="id" placeholder="Enter Id of entry to delete" maxlength="10">
      </div>
    </div>
   ...
   setcookie('del_id',$name,time()+36000);
   setcookie('del_option',$option,time()+36000);
   echo $z=@$_COOKIE['del_id'];
   echo $z=@$_COOKIE['del_option'];
   ...
   if($option == "Receipt")
   {
   	$query="SELECT * FROM `Sheet1` WHERE id='$name'";
		...
		echo @$x=$_POST['delForm'];

		if(isset($_POST['delForm']))
		{
			if($option == "Issue")
			{
				$query="DELETE from `Sheet2` WHERE id='$name'";
			}
			if($option == "Receipt")
			{
				$query="DELETE from `Sheet1` WHERE id='$name'";
			}
		}
	}
   }
   ...
   <form class="form-horizontal" name="delForm" method="post" action="delete_entry_log.php">
   <div class="form-group">
      <div class="col-sm-offset-1 col-sm-10">
        <button type="submit" class="btn btn-default" onclick="return confirm('Are you sure?')" >Delete Entry</button>
      </div>
    </div>
    </form>
   </body>
